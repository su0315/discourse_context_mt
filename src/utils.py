...
class GenerationMixin:
    ...
    def generate(
        self,
        inputs: Optional[torch.Tensor] = None,
        src_context_ids: Optional[torch.Tensor] = None, ### CZ
        src_context_attention_mask: Optional[torch.Tensor] = None, ### CZ
        tgt_context_ids: Optional[torch.Tensor] = None, ### CZ
        ...
        **model_kwargs,
        ...
        if is_greedy_gen_mode:
            ...
            return self.beam_search(
                input_ids,
                src_context_ids,
                src_context_attention_mask,
                tgt_context_ids, ### CZ pass context ids
                beam_scorer,
                logits_processor=logits_processor,
                stopping_criteria=stopping_criteria,
                pad_token_id=pad_token_id,
    ...
    def contrastive_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            if "src_context_attention_mask" in model_inputs:
                if (torch.count_nonzero(model_inputs["src_context_attention_mask"])>0):
                    model_inputs["attention_mask"] = model_inputs["src_context_attention_mask"]
    ...
    def beam_search(
        self,
        input_ids: torch.LongTensor,
        src_context_ids: torch.LongTensor,  ### CZ
        src_context_attention_mask: torch.LongTensor,  ### CZ
        tgt_context_ids: torch.LongTensor,  ### CZ
        ...
        **model_kwargs,
        ...
        while True:
            ...
            model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
            ### CZ: repeat the context for each beam 
            #print('inputs',input_ids.shape)
            if tgt_context_ids is not None and input_ids.shape[0]<=1:
                tgt_context_ids = torch.repeat_interleave(tgt_context_ids, num_beams, dim=0)
                model_inputs['tgt_context_ids']=tgt_context_ids
            #print("utils_inputs:",model_inputs.keys())
            
            if (src_context_ids is not None) and (src_context_attention_mask is not None):
                #print(input_ids.shape)
                if input_ids.shape[1]<=1:
                    src_context_ids = torch.repeat_interleave(src_context_ids, num_beams, dim=0)
                    src_context_attention_mask = torch.repeat_interleave(src_context_attention_mask, num_beams, dim=0)
                model_inputs['src_context_ids']=src_context_ids
                model_inputs['src_context_attention_mask']=src_context_attention_mask
