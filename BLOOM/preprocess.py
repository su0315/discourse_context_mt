...
def preprocess_function(src_lang, tgt_lang, tag, speaker, src_context_size, tgt_context_size, random_context, cw_dropout_rate, tgt_sep, tokenizer, data): 
        ...
        # Randomely decide True or False for CXMI random speaker model
            ...
            if source_context_size == 0:
                ...
                if len(context_list) ==0:
                    ...
                    concat_contexts = "</t>".join(context_list) # SU: To delete the </t> between contexts
    ...
    if tgt_context_size>0 or random_context: 
        ...
        print ("len(context_ids)", len(model_inputs["tgt_context_ids"]))
    ...
    if src_context_size > 0 or tag or speaker: # When random speaker and scene model, we do CoAtt model
        ...
        for c,i in zip(src_context_attn,model_inputs['attention_mask']):
            ci = [0]*len(c) # mask context attention as "0"
            c.extend(i) # extend input attention score after context attention score 
            ci.extend(i) # extend input attention score after masked context attention scores (0)
            new_attentions.append(c) # non masked 
            new_src_attentions.append(ci) # context masked
    ...
    print("len(laels)", len(model_inputs['labels']))
    print("len(input_ids)", len(model_inputs['input_ids']))
    print("len(att)", len(model_inputs['attention_mask']))
